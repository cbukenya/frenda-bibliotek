        ...
        <div className="book-detail__hero">
          ...
          <div>
            ...
            <div className="book-detail__meta">
              <div><div className="meta-item__label">{t('isbn')}</div><div className="meta-item__value">{book.isbn}</div></div>
              <div><div className="meta-item__label">{t('genre')}</div><div className="meta-item__value">{book.genre}</div></div>
              <div><div className="meta-item__label">{t('pages')}</div><div className="meta-item__value">{book.totalPages}</div></div>
              <div><div className="meta-item__label">{t('year')}</div><div className="meta-item__value">{book.publishedYear}</div></div>
            </div>

            {book.avgReadingDays && (
              <div className="reading-time-banner">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                {t('avgLoan', { days: Math.round(book.avgReadingDays), time: estReadingHours(book.totalPages) })}
              </div>
            )}
          </div>
        </div>
        ...
        {book.recommendations.length > 0 && (
          <div className="recommendations">
            <div className="section-header">
              <h2 className="section-title">{t('recommendationsTitle')}</h2>
            </div>
